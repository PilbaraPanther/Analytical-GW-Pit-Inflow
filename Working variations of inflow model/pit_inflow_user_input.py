...
def pit_inflow_user_input():
    #user input

    width=float(input('Width of pit (m):'))
    length=float(input('Length of pit (m):'))
    W = float(input('Enter distributed recharge (m/d):'))
    kh1=float(input('zone 1 horizontal K (m/d):'))
    kh2=float(input('zone 2 horizontal K (m/d):'))
    kv=float(input('zone 2 vertical K (m/d):'))
    pitbase=float(input('pit base elevation (mRL):'))
    sGWL=float(input('Static GWL (mRL):'))

    #return variables
    return width, length, W, kh1, kh2, kv, pitbase, sGWL

   
def pit_inflow(width, length, W, kh1, kh2, kv, pitbase, sGWL):
    ...
    dt1 = dt.split('.')
    dt2=dt1[0].replace(':','-')
    output = 'K_'+ K + 'Recharge_' + distrutedRecharge+ '_pit_inflow_output_'+ datetimesave+'.csv'
