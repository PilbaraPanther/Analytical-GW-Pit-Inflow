...
#RE: RAINFALL AND RECHARGE AND HOW IT WORKS IN MODEL
...
def pit_inflow():
    ...
    results = {'recharge':[],'R0_m':[],'zone_1_pit_inflow_m3/d':[],'drawdown_m':[],'zone_2_pit_inflow_m3/d':[],
                       'total_pit_inflow_m3/d':[],'pit_level_mRL':[]}
    
    #create distribution of rainfall recharge
    #a is skewness, loc is mean (mm/d), scale is standard deviation, size in numbers of random numbers
    rainfall = skewnorm.rvs(a=8.5, loc= 2.5, scale= 8.7, size=10)
    percRecharge = 0.04 # % of rainfall
    #create loop to run for each different recharge volume
    for rain in rainfall:
        recharge = rain/1000* percRecharge
    ...
    dt1 = dt.split('.')
    dt2=dt1[0].replace(':','-')
    output = 'K_'+ K + 'random rain'+ '_pit_inflow_output_'+ dt2+'.csv'

    #put dictionary into dataframe and then export dataframe to .csv file
    df=pd.DataFrame.from_dict(results,'columns')
    df.sort_values(by='R0_m',ascending=True,inplace=True)
    df.to_csv(output,sep=',',columns=['recharge','R0_m','zone_1_pit_inflow_m3/d','zone_2_pit_inflow_m3/d',
    ...
    #chart
    #fig, ax = plt.subplots()
    ax = df.plot(x='total_pit_inflow_m3/d', y='pit_level_mRL',)
